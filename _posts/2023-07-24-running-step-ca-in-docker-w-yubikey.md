...
**_NOTE_** There is currently a bug in step-ca's `dockerfile.hsm`. See the final step of the guide for a quick fix.
...
**_I'm hoping that this section becomes fixed in the future. Track the PR here: <insert PR here>_**
...
The above _should_ work, but if it doesn't, try running the container as root user with the option `-u 0`
